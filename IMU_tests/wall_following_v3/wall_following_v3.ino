...
// GOOD VARIANT double Kp = 0.8, Ki = 0, Kd = 2.5;
double Kp = 0.8, Ki = 0, Kd = 2.5;
...
void loop() {
  ...
  delay(500);
  // delay(10) // actual competition delay
}

void moveRobot() {
  // Read distances
  // distLeftBuffer[filterIndex] = sensorLeft.readRangeContinuousMillimeters() - leftOffset;
  // distRightBuffer[filterIndex] = sensorRight.readRangeContinuousMillimeters() - rightOffset;
  // filterIndex = (filterIndex + 1) % FILTER_SIZE;

  // Compute filtered distances
  // distLeft = 0; distRight = 0;
  // for (int i=0; i<FILTER_SIZE; i++) {
  //   distLeft += distLeftBuffer[i];
  //   distRight += distRightBuffer[i];
  // }
  // distLeft /= FILTER_SIZE;
  // distRight /= FILTER_SIZE;
  double alpha = 0.3;  // Smoothing factor (0.2-0.5)
  distLeft = sensorLeft.readRangeContinuousMillimeters();  
  distRight = sensorRight.readRangeContinuousMillimeters();
  distLeft = alpha * (distLeft - leftOffset) + (1 - alpha) * distLeft;
  ...
  int PWM_Left = constrain(baseSpeed - correction, 90, maxPWM);
  int PWM_Right = constrain(baseSpeed + correction, 90, maxPWM);
