...
void loop() {
  if (isFirstTurn) {
    turnRobot(125.0); // greater offset needed for first turn - or maybe motors need an initial kick
    isFirstTurn = 0;
  }
  else {
    turnRobot(95.0); // turn 90 degrees (+ offset)
  }
  delay(3000);
}
...
void turnRobot(float targetAngle) {
  myPID.SetMode(MANUAL);  // Reset internal PID state - ensures no carry-over from previous turns
  myPID.SetMode(AUTOMATIC);
  angle = 0; // Reset angle
...
void driveMotors(double pidOutput) {
  int leftPower = constrain(pidOutput, -160, 160);
  int rightPower = constrain(-pidOutput, -160, 160);
